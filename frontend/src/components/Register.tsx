...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await api.post('/auth/v1/register', { name, email, password });
      // Usually register might return user without token, let's login right after
      if (response.data.token) {
        login(response.data.token, response.data.user);
        navigate('/');
      } else {
        const loginRes = await api.post('/auth/v1/login', { email, password });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50/50 p-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <p className="text-center text-gray-500 mb-8">Start managing your alerts effortlessly.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-blue-600 text-white font-medium py-3 rounded-xl hover:bg-blue-700 transition-colors focus:ring-4 focus:ring-blue-600/20 disabled:opacity-70 flex items-center mt-2 justify-center"
          >
