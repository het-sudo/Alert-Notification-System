...
import { LogOut, Bell, Trash2, CheckCircle2, AlertCircle, Info, Plus, MailOpen, Mail } from 'lucide-react';
import api from '../api';
...
export default function Dashboard() {
  ...
  const [filter, setFilter] = useState<'all' | 'read' | 'unread' | 'external'>('all');
  ...
  const toggleRead = async (id: string, currentIsRead: boolean) => {
    try {
      ...
      setAlerts((prev) => {
        const updated = prev.map(a => a._id === id ? { ...a, isRead: newIsRead } : a);
  ...
  const createAlert = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post('/alerts/v1', { title: newAlertTitle, message: newAlertMessage, type: newAlertType });
    ...
    <div className="min-h-screen bg-gray-50/50 pb-12">
      ...
      <nav className="bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm">
        <div className="flex items-center space-x-3">
          ...
          <span className="font-semibold text-lg text-gray-900 tracking-tight">AlertHub</span>
        </div>
      ...
      <main className="max-w-4xl mx-auto mt-8 px-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900">Your Notifications</h1>
            <p className="text-sm text-gray-500 mt-1">Stay updated with the latest alerts</p>
          </div>
          <div className="flex items-center space-x-3">
            <button
        ...
        {isAddingMode && (
          <form onSubmit={createAlert} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 mb-8 animation-fade-in">
            ...
            <div className="flex justify-end gap-3">
              <button type="button" onClick={() => setIsAddingMode(false)} className="px-5 py-2 text-sm text-gray-500 hover:text-gray-700 font-medium">Cancel</button>
              <button type="submit" className="px-5 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium shadow-sm shadow-blue-600/20 transition-colors">Publish Alert</button>
            </div>
          </form>
        )}
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            <div className="text-center py-16 bg-white rounded-2xl border border-gray-200 border-dashed">
              ...
              <p className="text-sm text-gray-400 mt-1">No alerts found for this filter.</p>
            </div>
          ) : (
            alerts.map((alert) => (
              <div
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-1 gap-2">
                    <h3 className={clsx("font-semibold truncate", alert.isRead ? "text-gray-500" : "text-gray-900")}>
                      {alert.title}
                    </h3>
                  ...
                  <p className={clsx("text-sm", alert.isRead ? "text-gray-400" : "text-gray-600")}>
                    {alert.message}
                  </p>
                </div>
